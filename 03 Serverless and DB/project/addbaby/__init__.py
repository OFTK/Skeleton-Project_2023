...
def main(req: func.HttpRequest, signalRMessages: func.Out[str]) -> func.HttpResponse:
    ...
    try:
        # access table called "families", check if family exists (family name is RowKey), if not - add it to the table
        with TableClient.from_connection_string(connection_string, table_name="families") as table:
            get_family_name_query = f"RowKey eq '{family}'"
            logging.error(f"looking for family name with query {get_family_name_query}")
            entities = table.query_entities(get_family_name_query)
            family_not_found = True
            for entity in entities:
                entity_name = entity["RowKey"]
                logging.info(f"found family with name {entity_name}")
                # if family name exists - return error
                if entity_name == family:
                    logging.warning(f"family name in request {family} already exists in DB")
                    family_not_found = False
            
            # if family name not found - add it to the DB
            if family_not_found:
                logging.warning(f"trying to add {family} to the DB")
                new_family_entity = {
                    u'PartitionKey': u'families',
                    u'RowKey': u'{}'.format(family)
                }
                new_entity = table.create_entity(entity=new_family_entity)
